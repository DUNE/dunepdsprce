...
# Retreive the location of the dunepdsprce repository
# Git checkout if blank or "git".
PKGDIR=$1
if [ "$PKGDIR" = "git" ]; then PKGDIR=; fi
if [ -n "$PKGDIR" ]; then
  PKGDIR=$(readlink -f $PKGDIR)
  echo $MYNAME Package directory: echo $PKGDIR
fi

# Retrive the installation directory.
# No install if blank.
DSTDIR=$2
...
INSTALLCOM=
if [ -n "DSTDIR" ]; then
  FILE=$(dirname $0)/installPackage
  INSTALLCOM=$(readlink -f $FILE)
fi
...
if [ -z "PKGDIR" ]; then
  echo $MYNAME Checking out package
...
echo $MYNAME Copying libararies...
cd ${PKGDIR}/dam/source/cc/make || exit 1
make clean || exit 1
...
echo $MYNAME Copying binaries...
cp -R -L ${PKGDIR}/install/x86_64-${SIMDQUALIFIER}-${LINDAR}/bin/* ${CURDIR}/${PRODUCT_NAME}/${VERSION}/${FLAVOR}-${QUAL}-${SIMDQUALIFIER}-${BUILDTYPE}/bin
...
echo $MYNAME Copying libararies...
cd ${PKGDIR}/dam/export/x86_64-${SIMDQUALIFIER}-${LINDAR}/lib
...
cp -R -L ${PKGDIR}/install/x86_64-${SIMDQUALIFIER}-${LINDAR}/include/* ${CURDIR}/${PRODUCT_NAME}/${VERSION}/include || exit 1
...
TARFILE=$WORKSPACE/copyBack/${FULLNAMESTRIPPED}.tar.bz2
tar -cjf $TARFILE .
...
if [ -n "$INSTALLCOM" ]; then
  if [ -x "$INSTALLCOM" ]; then
    echo Installing in $DSTDIR
    $INSTALLCOM $TARFILE $DSTDIR
  else
    echo "Unable to find/run install script: ./installPackage"
  fi
fi
exit 0
